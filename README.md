...
- **Nostr → Solid** — `window.nostr.getPublicKey()` (or paste a 64-char hex key, or **generate a fresh one**) → resolves
  ...
  Keys are **64-char hex** (did:nostr-native) — no `nsec`/`npub`.
- **Solid → Nostr** — paste a WebID or pod URL → reads the profile → finds a linked
  `did:nostr` / `npub`.
- **Guides** — a 3-step coach with the next concrete action; if you have no storage it recommends a
  hosted pod at **[solidweb.app](https://solidweb.app/idp/register)** (or self-host with `npx jspod`), then a re-check.
